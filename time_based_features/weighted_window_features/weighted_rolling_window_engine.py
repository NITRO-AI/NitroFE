            ...
            if 'weighted_window_features' in self.payload[_column_key].keys():
                ...
                for _window_keys in self.payload[_column_key]['weighted_window_features'].keys():
                    if (_window_keys in self.function_mapper.keys()):
                        ...
                        dict_all = self.payload[_column_key]['weighted_window_features'][_window_keys]

                        feature_generated = []
                        if isinstance(self.payload[_column_key]['weighted_window_features'][_window_keys]['window'],int):
                            iter_duration=1
                            resulting_feature = self.function_mapper[_window_keys](self.dataframe[_column_key], **self.payload[_column_key]['weighted_window_features'][_window_keys] )
                            feature_generated.append(resulting_feature)

                        else:
                            iter_duration=len(self.payload[_column_key]['weighted_window_features'][_window_keys]['window'])
                            for _iter in range(iter_duration):
                                resulting_feature = self.function_mapper[_window_keys](self.dataframe[_column_key], **dict(zip(dict_all.keys(),np.array(list(dict_all.values()))[:,_iter])) )
                                feature_generated.append(resulting_feature)
